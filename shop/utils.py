...
def get_sales_metrics(days=30):
    ...
    for order in daily_orders:
        ...
        daily_sales.setdefault(date_str, {'date': date_str, 'total_sales': 0, 'orders_count': 0})
    ...
    for order in monthly_orders:
        ...
        monthly_sales.setdefault(month_str, {'month': month_str, 'total_sales': 0, 'orders_count': 0})
    ...
    monthly_sales = sorted(monthly_sales.values(), key=lambda x: x['month'], reverse=True)[:12]
    ...
    top_products = [{'product': p['product__name'], 'quantity': p['quantity']} for p in top_products_qs]
